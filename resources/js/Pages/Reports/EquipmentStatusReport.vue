...
const status= ref('Good condition');
const dateRange= ref('');
const date_option = ref('All');

function generateReport() {
    Inertia.get(route('reports.equipment-status.generate'), {
        status: status.value,
...
<template>
    <Head title="Equipment Status Report" />
    <div
        ...
        <form @submit.prevent="generateReport">
            <div class="flex justify-between">
                <div>
                    ...
                    <p class="text-md text-gray-700">List of status of Equipments</p>
                </div>
                <button
                    type="submit"
            ...
            <div class="grid grid-cols-1 mt-5 p-3 gap-5 md:w-1/3">
                <div class="col-span-1">
                    <label class="block mb-2 font-medium text-md text-gray-800 dark:text-white" for="status">Select Status:</label>
                    <select
                    v-model="status"
