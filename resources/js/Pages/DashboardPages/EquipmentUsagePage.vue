...
const showFilterTypeDropdown = ref(false);
...
onMounted(() => {
    ...
    if (props.equipmentUsageChart && props.equipmentUsageChart.length > 0) {
        adjustChartData(props.equipmentUsageChart);
        setupChart();
    } else {
...
watch(() => props.equipmentUsageChart, (newData) => {
...
const adjustChartData = (data) => {
    ...
    if (filterType.value === 'by_dates') {
        if (diffDays >= 30) {
            ...
            equipmentUsageLabels.value = groupedData.labels;
            equipmentUsageData.value = groupedData.data;
        } else {
            equipmentUsageLabels.value = data.map((item) => item.date);
            equipmentUsageData.value = data.map((item) => item.quantity);
        }
    } else if (filterType.value === 'by_equipments') {
        const equipmentData = aggregateEquipmentUsage(data);
        equipmentUsageLabels.value = equipmentData.labels;
        equipmentUsageData.value = equipmentData.data;
    }
};
...
const aggregateEquipmentUsage = (data) => {
    const equipmentData = {};
    data.forEach((item) => {
        const { equipments } = item;
        if (equipments) {
            Object.keys(equipments).forEach(equipmentName => {
                if (!equipmentData[equipmentName]) {
                    equipmentData[equipmentName] = 0;
                }
                equipmentData[equipmentName] += equipments[equipmentName];
            });
        }
    });
...
const setupChart = () => {
    ...
    window.myChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: filterType.value === 'by_dates' ? 'Equipment Consumption by Date' : 'Equipment Consumption by Equipment',
                data: equipmentUsageData.value,
                borderColor: selectedChartType.value === 'line' ? 'rgba(147, 197, 253, 1)' : 'rgba(147, 197, 253, 0)',
        ...
        options: {
            ...
            scales: selectedChartType.value === 'pie' ? {} : {
                y: {
                    ...
                    title: {
                        display: true,
                        text: filterType.value === 'by_dates' ? 'Quantity' : 'Equipment Quantity',
                        font: {
                            size: 16,
                            weight: 'bold',
                            style: 'italic'
                ...
                x: {
                    title: {
                        display: true,
                        text: filterType.value === 'by_dates' ? 'Dates' : 'Equipment',
                        font: {
                            size: 16,
                            weight: 'bold',
                            style: 'italic'
...
const sortedequipmentlog = computed(() => {
    const sortedLogs = [...props.equipmentlog.data]; 
...
<template>
    <Head title="Equipment Consumption Log" />
    ...
    <div class="relative-container bg-white dark:bg-gray-800 relative overflow-x-auto shadow-md sm:rounded-lg">
                    ...
                    <div v-if="showDropdown" class="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg z-50 border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                        <div class="p-4 space-y-4">
                            <select v-model="filterType" @change="filterLogs" class="w-full bg-gray-50 border text-gray-900 text-sm rounded-lg p-2.5">
                                ...
                                <option value="by_equipments">By Equipments</option>
                            </select>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="date" v-model="form.start_date" placeholder="From" class="w-full bg-gray-50 border text-gray-900 text-sm rounded-lg p-2.5">
                                <input type="date" v-model="form.end_date" placeholder="To" class="w-full bg-gray-50 border text-gray-900 text-sm rounded-lg p-2.5">
        ...
        <div class="h-72 p-4">
            <p v-if="filterType === 'by_equipments'" class="text-l text-gray-600 mt-2 text-center font-bold text-black bg-gray-50 dark:bg-gray-700">
                {{ formattedDateRange }}
            </p>
        ...
        <div class="relative overflow-x-auto sm:rounded-lg mt-4 hidden md:block">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody v-if="sortedequipmentlog.length === 0">
                    <tr>
                        <td
                            colspan="8"
                ...
                <tbody>
                    <tr v-for="(log, index) in sortedequipmentlog" :key="index" class="bg-white border-b hover:bg-gray-50">
                        <td class="px-6 py-4 font-medium text-black">{{ log.equipment_name }}</td>
        ...
        <div class="relative overflow-hidden sm:rounded-lg mt-4 block md:hidden">
            <div class="grid grid-cols-1 gap-4">
                <div v-for="(log, index) in sortedequipmentlog" :key="index" class="bg-white shadow-md rounded-lg p-4 border">
                    <div class="flex items-start mb-2">
                        <h3 class="font-medium text-black">{{ log.equipment_name }}</h3>
                    </div>
        ...
        <Pagination :paginator="equipmentlog" />
    </div>
</template>
