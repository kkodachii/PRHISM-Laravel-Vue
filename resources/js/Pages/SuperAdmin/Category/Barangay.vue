...
const props = defineProps({
    barangays: Object,
    unusedBarangays: Array,
});

const unusedBarangays = ref(props.unusedBarangays);
watch(() => props.unusedBarangays, (newVal) => {
    unusedBarangays.value = newVal;
}, { deep: true });
...
watch(
    search,
    debounce(
        (q) => router.get("/barangay", { search: q }, { preserveState: true }),
        500
    )
);
...
function startEditing(barangay) {
    isAdding.value = false;
    openEditing.value = true;
    isEditing.value = barangay.id;
    editForm.barangay_name = barangay.barangay_name;
    editForm.id = barangay.id;
    editForm.rhu_id = barangay.rhu_id;
}
...
function submitAdd() {
    addForm.post(route("barangay.store"), {
        onSuccess: () => {
            addForm.reset();
            isAdding.value = false;
            openEditing.value = false;
...
function submitEdit() {
    editForm.put(route("barangay.update", editForm.id), {
        onSuccess: () => {
            editForm.reset();
            isAdding.value = false;
            openEditing.value = false;
...
function submitDelete(barangay) {
    deleteForm.delete(route("barangay.destroy", { id: barangay.id }), {
        onSuccess: () => {
            deleteForm.reset();
            isAdding.value = false;
            openEditing.value = false;
...
<template>
    ...
    >
        <Delete :ID="selectedID" :item="'brgy'" />
    </div>
            ...
            <form @submit.prevent="submitAdd">
                ...
                >
                    ...
                    <div class="flex lg:flex-row flex-col gap-3">
                        ...
                        />
                        <select
                            v-model="addForm.rhu_id"
                            required
                            id="rhu_id"
                            class="block w-full lg:w-5/12 p-2.5 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        >
                    ...
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ addForm.errors.rhu_id }}</p>
                </div>
            </div>
            </form>
                                </template>
            ...
            >
                ...
                <div class="flex lg:flex-row flex-col gap-3">
                    <input
                        v-model="editForm.barangay_name"
                        type="text"
                        name="barangay_name"
                        id="barangay_name"
                        class="block w-full lg:w-5/12 p-2.5 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
            ...
            <div class="relative overflow-x-auto sm:rounded-lg">
                ...
                >
                    ...
                    >
                        <tr>
                            ...
                            <th scope="col" class="px-8 w-48 py-3">Barangay Name</th>
                            <th scope="col" class="px-2 w-48 py-3">RHU</th>
                ...
                >
                    ...
                    <tbody v-for="barangay in barangays.data">
                        ...
                        >
                            ...
                            <th class="px-2 w-48 py-4">
                                {{ barangay.rhu.rhu_name }}
                            </th>
                            <td class="px-2 w-80 py-4">
                                <div class="flex justify-center">
                                    ...
                                    >
                                        ...
                                        <button
  v-if="unref(unusedBarangays).some(item => item.id === barangay.id)"
  data-modal-target="crud-modal3"
                                                        data-modal-toggle="crud-modal3"
  @click="openDeleteModal(barangay.id)"
...
>
  Delete
            ...
            <Pagination :paginator="barangays" />
        </div>
    </div>
</template>
