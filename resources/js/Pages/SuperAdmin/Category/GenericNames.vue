...
const props = defineProps({
    generic_names: Object,
    unusedGenericNames: Array,
    categories: Object,
});
...
const unusedNames = ref(props.unusedGenericNames);
...
watch(
    search,
    debounce(
        (q) => router.get("/generic_name", { search: q }, { preserveState: true }),
        500
    )
);
...
function startEditing(generic_name) {
    isAdding.value = false;
    openEditing.value = true;
    isEditing.value = generic_name.id;
    editForm.generic_name = generic_name.generic_name;
    editForm.id = generic_name.id;
}
...
function submitAdd() {
    addForm.post(route("generic_name.store"), {
        onSuccess: () => {
            addForm.reset();
            isAdding.value = false;
            openEditing.value = false;
...
function submitEdit() {
    editForm.put(route("generic_name.update", editForm.id), {
        onSuccess: () => {
            editForm.reset();
            isAdding.value = false;
            openEditing.value = false;
...
function submitDelete(generic_name) {
    deleteForm.delete(route("generic_name.destroy", { id: generic_name.id }), {
        onSuccess: () => {
            deleteForm.reset();
            isAdding.value = false;
            openEditing.value = false;
...
watch(() => props.unusedGenericNames, (newVal) => {
    unusedNames.value = newVal;
}, { deep: true });
...
<template>
    ...
    >
        <Delete :ID="selectedID" :item="'genname'" />
    </div>
                <div
                ...
                >
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div class="col-span-1">
                            <InputLabel for="category_id" value="Medical Category" />
                            <select
                        ...
                        <div class=" col-span-1">
                            <InputLabel for="generic_name" value="Generic Name" />
                            <input
            ...
            <div class="relative overflow-x-auto sm:rounded-lg">
                ...
                >
                    ...
                    >
                        <tr>
                            <th scope="col" class="px-8 w-48 py-3">Category</th>
                            <th scope="col" class="px-2 w-48 py-3">Generic Name</th>
                            <th scope="col" class="px-2 w-80 text-center py-3">
                                Action
                            </th>
                        </tr>
                    </thead>
                    <tbody v-for="generic_name in generic_names.data">
                        ...
                        >
                            ...
                            <th class="px-8 w-48 py-4">
                                {{ generic_name.category.category}}
                            </th>
                            ...
                            <td class="px-2 w-80 py-4">
                                <div class="flex justify-center">
                                    ...
                                    <button
                                        v-else
                                        @click="startEditing(generic_name)"
                                    ...
                                    <button
                v-if="unref(unusedNames).some(item => item.id === generic_name.id)"
                data-modal-target="crud-modal3"
                                                        data-modal-toggle="crud-modal3"
  @click="openDeleteModal(generic_name.id)"
            ...
            <Pagination :paginator="generic_names" />
        </div>
    </div>
</template>
