...
const invSetting = ref(false);
const reportsSetting = ref(false);

function invSettingOpen() {
    if (invSetting.value == false) {
        invSetting.value = true;
    } else {
        invSetting.value = false;
    }
}

function reportsSettingopen() {
    if (reportsSetting.value == false) {
        reportsSetting.value = true;
    } else {
        reportsSetting.value = false;
    }
}
</script>
<template>
    <Sidebar></Sidebar>
    <div class="p-5 mb-20">
        <div class="flex flex-col items-center gap-1 mb-10 mt-5">
            <img
                class="w-20 h-20 mb-3 rounded-full"
                :src="
        ...
        >
            ...
            <li v-if="user.role_id === 3">
                <Link
                    ...
                    :class="{
                        'bg-gray-100': $page.url.split('?')[0] === '/backups',
                    }"
                    class="flex items-center p-2 text-base font-medium text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group"
                >
                    <svg
