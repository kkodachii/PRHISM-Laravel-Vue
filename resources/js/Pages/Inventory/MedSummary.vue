...
<template>
    ...
    <div class="bg-white dark:bg-gray-800 relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <div class="block md:hidden">
            <div v-for="mstatus in Medstatus.data" :key="mstatus.id" class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md border-b dark:border-gray-700 relative">
                <div class="flex flex-row justify-start">
                    <div class="flex items-center md:w-10 w-10 mx-4">
                        <img v-if="mstatus.type === 'Medicine'" src="/storage/icons/midwife/pill.svg" alt="Medicine Icon" />
                        <img v-if="mstatus.type === 'Equipment'" src="/storage/icons/midwife/microscope.svg" alt="Equipment Icon" />
                        <img v-if="mstatus.type === 'Medical Supply'" src="/storage/icons/midwife/bandaid.svg" alt="Medical Supply Icon" />
                    </div>
                    <div class="flex flex-col flex-grow">
                        ...
                        <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                            <template v-if="mstatus.brand">{{ mstatus.brand }}</template>
                            <template v-else>{{ mstatus.description }}</template>
                        </div>
                        <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                            Quantity: {{ mstatus.quantity }}
                        </div>
                    </div>
                </div>
                <div class="absolute top-4 right-4 whitespace-nowrap">
                    <StatusLabel :all="mstatus.status" />
                </div>
            </div>
        </div>
        ...
        <div class="hidden md:block">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    <tr v-for="mstatus in Medstatus.data" :key="mstatus.id" class="bg-white border-b text-gray-900 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer">
                        <td class="flex items-center px-8 p-4 mr-12 space-x-6 whitespace-nowrap">
                            <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                                <div class="text-base font-semibold text-gray-900 dark:text-white">{{ mstatus.name }}</div>
                                <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                                    <template v-if="mstatus.brand">{{ mstatus.brand }}</template>
                                    <template v-else>{{ mstatus.description }}</template>
                                </div>
        ...
        <Pagination :paginator="Medstatus"  :status="selectedstatus" />
    </div>
</template>
