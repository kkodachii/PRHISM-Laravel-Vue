...
// Form setup
const form = useForm({
    id: props.med_supply?.id ?? "",
    user_id: props.med_supply?.user_id ?? "1",
    batch_id: props.med_supply?.batch_id ?? "",
    barangay_id: props.med_supply?.barangay_id ?? "1",
    med_sup_name: props.med_supply?.med_sup_name ?? "",
    description: props.med_supply?.description ?? "",
    quantity: props.med_supply?.quantity ?? "",
    provider: props.med_supply?.provider ?? "",
    date_acquired: props.med_supply?.rawDateAcquired ?? "",
});
...
// Watch for med_supply changes to update form
watch(
    () => props.med_supply,
    (newVal) => {
        if (newVal) {
            ...
            form.med_sup_name = newVal.med_sup_name || "";
...
function submit() {
    ...
    form.put(route("medical_supplies.update", props.med_supply.id), {
        onSuccess: () => {
                ...
                <div class="grid gap-4 mb-4 grid-cols-3">
                    <div class="col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <!-- Dropdown menu -->
                            <div
                                v-show="showDropdown && filteredSupplies.length"
                            ...
                            >
                                <ul class="max-h-48 overflow-y-auto">
                                    <li
                                        ...
                                        :key="supply.id"
                                        @click="
                                            selectSupply(supply.med_sup_name)
                                        "
